...
class SSMKernelDPLR(SSMKernelDiag):
    ...
    def _step_state(self, u, state:torch.Tensor):
        """Must be called after self.default_state() is used to construct an initial state!"""
        if state.dtype == torch.torch.float32:
    ...
    def step(self, u, state):
        ...
        # new_state = torch.complex(new_state, torch.zeros_like(new_state))
